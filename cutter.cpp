...
static int save_frame_to_png(AVFrame* frame, std::string filename);
...
int main(int argc, const char* argv[])
{
	...
	for (int i = 0; i < format_context->nb_streams; i++)
	{
		...
		local_codec = const_cast<AVCodec*>(avcodec_find_decoder(local_codec_parameters->codec_id));
...
static int decode_packet(AVPacket* input_packet, AVCodecContext* codec_context, AVFrame* input_frame)
{
	...
	while (ret >= 0)
	{
		...
		if (ret >= 0)
		{
			...
			std::string frame_filename = fmt::format("output/frame-{0}.png", codec_context->frame_num);
